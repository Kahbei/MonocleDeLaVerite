export const questions = [
    {
        question: "Les Américains n’ont jamais marché sur la Lune",
        type: "unique",
        reponse: 1,
        explication:
            "Les rumeurs sur le programme Apollo sont une théorie du complot selon laquelle les vaisseaux du programme Apollo ne se seraient jamais posés sur la Lune et qu’il s’agirait en fait d’une mise en scène réalisée sur Terre par la NASA. Les preuves photographiques et les films auraient été fabriqués de toutes pièces, y compris les prélèvements de roches sur la Lune. Les adeptes de la théorie du complot invoquent le fait que le drapeau flotte au vent alors qu’il n’y a pas d’air, que les réticules de cadrage apparaissent curieusement masqués, ou encore que les astronautes et les objets restent étonnement bien visibles dans les ombres… Des théories balayées par des preuves, elles, bien argumentées.",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://trendly.fr/wp-content/uploads/sites/3/2016/08/theorie-complot-homme-marcher-lune.jpg",
    },
    {
        question: "Diana, la princesse de Galles, a été assassinée ?",
        type: "unique",
        reponse: 1,
        explication:
            "Malgré une enquête officielle qui n’a trouvé aucune preuve d’un complot par le MI6 ou toute autre entité pour assassiner la princesse Diana et Dodi Fayed en 1997, une fiévreuse spéculation continue autour de ce drame. La théorie veut que des “éléments” dans les services secrets britanniques aient décidé que la relation entre Diana et Fayed était une menace pour la monarchie et, par conséquent, pour l’Etat britannique. Un complot a été établi dans lequel une Fiat Uno blanche transportant des agents a été envoyée pour aveugler et désorienter le chauffeur Henri Paul, alors qu’il roulait dans le tunnel situé au niveau du pont de l’Alma. Plus tard, l’échantillon du sang de Henri Paul aurait été échangé avec celui de quelqu’un ayant bu beaucoup d’alcool. Le problème avec cette théorie ? Pas la moindre preuve existe pour la soutenir.",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://trendly.fr/wp-content/uploads/sites/3/2016/08/theorie-complot-princesse-diana.jpg",
    },
    {
        question:
            "Quels arguments pourraient contredire cette idée : La Terre est plate car quand je regarde l'horizon je ne vois pas la courbure de la terre, plusieurs choix peuvent être attendu",
        type: "multiple",
        reponse: [0, 2],
        explication:
            "Cet argument se base sur le fait qu'on ne peut très rarement voir la courbure de la terre cela est dû à la nôtre taille par rapport à la Terre. Pour le démontrer vous pouvez faire l'expérience suivante : Prenez un globe est placez y une caméra a sa surface vous constaterez la courbure de la terre s'aplanie. A l'opposé plus on s'élève de la surface plus on peut voir la courbure de la terre. On ne peut pas utiliser l'argument B car il pourrait contre argumenter avec un défaut de l'objectif de l'appareil (Pour plus de débunkage sur ce sujet voir : 🌎 La Terre est ronde. C'est ton encéphalogramme qui est plat. DEFAKATOR : https://youtu.be/g-2p2Ahrfts)",
        proposition: [
            {
                id: 0,
                text: "Nous ne sommes pas à la bonne échelle pour pouvoir voir la courbure de la terre",
            },
            {
                id: 1,
                text: "Des photos montres la courbure de la terre",
            },
            {
                id: 2,
                text: "La courbure de la terre est visible lorsqu'on voit disparaitre les objets à l'horizon",
            },
            {
                id: 3,
                text: "La réponse D",
            },
        ],
        image: "https://pbs.twimg.com/media/EshhQS7VoAIcEe4?format=jpg&name=small",
    },
    {
        question: "Elvis Presley a-t-il simulé sa propre mort ?",
        type: "unique",
        reponse: 1,
        explication:
            "Une croyance persistante dit que The King n’est pas mort en 1977. Beaucoup de fans persistent à prétendre qu’il est encore vivant, qu’il est entré dans la clandestinité pour des raisons diverses. Cette affirmation est soutenue par des milliers de prétendues apparitions. La principale raison invoquée à propos du fait que Presley ait falsifié sa mort est que, sur sa tombe, son prénom Aron est orthographié Aaron. Mais Aaron est en fait le véritable prénom de Presley. Deux tabloïds ont publié des articles portant sur la vie continue de Presley après sa mort, dans les moindres détails. L’un des articles rapporte notamment qu’il s’est cassé une jambe dans un accident de moto, et ces journaux relatent des faits de sa vie jusqu’à sa prétendue mort réelle dans le milieu des années 1990.",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://cropper.watch.aetnd.com/cdn.watch.aetnd.com/sites/2/2018/05/hith-elvis-presley-53203711.jpg",
    },
    {
        question: "Quelles sont les principales recherches de la zone 51 ?",
        type: "unique",
        reponse: 3,
        explication:
            "La célèbre zone 51 qui se trouve dans le désert du Nevada est une installation militaire de la zone d’opérations militaires de Nellis. La base est l’un des sites militaires les plus secrets du monde et elle ne peut être observée sur aucune carte ni aucun site Web du gouvernement. Bien évidemment, tant de secrets ont échauffé les esprits et cela a conduit de nombreuses personnes à élaborer des théories du complot sur la fameuse zone 51. La théorie la plus populaire concernant cet endroit dit qu’on y fait des recherches, voir des expériences sur des aliens qui auraient eu la malchance d’arriver sur Terre. Quoi qu’il en soit, personne ne sait avec certitude ce qu’est la zone 51. Le gouvernement n’a même jamais reconnu publiquement l’existence de la base avant 2013, Officiellement, l’installation est utilisée pour tester des avions expérimentaux de l’armée.",
        proposition: [
            {
                id: 0,
                text: "Les reliques Incas",
            },
            {
                id: 1,
                text: "Le Saint Graal",
            },
            {
                id: 2,
                text: "Les Aliens",
            },
            {
                id: 3,
                text: "Militaires",
            },
        ],
        image: "https://lecafedugeek.fr/wp-content/uploads/2019/07/zone-51.jpg",
    },
    {
        question: "Ce image montre-t-elle un document officiel du gouvernement ?",
        type: "unique",
        reponse: 1,
        explication:
            "Ce document est complètement faux, pour le voir on peut d'abord vérifier les fautes d'orthographe ensuite on peut regarder la référence du document enfin il faut vérifier les sources du document afin de voir si il est sourcé . Plus d'information sur la création du document sur la vidéo suivante : https://www.youtube.com/watch?v=OOOkt2_PWc4",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://www.rts.ch/2020/12/07/16/18/11806422.image?w=640&h=640",
    },
    {
        question: "Comment le virus du SIDA a été transmis à l'homme ?",
        type: "unique",
        reponse: 3,
        explication:
            "Le sida est une pandémie qui a fait trembler le monde à la fin des années 1970, mais la première apparition de la maladie a eu lieu dans les années 1920. En 2014, il a été officiellement découvert que les premières souches de la maladie se sont développées à Kinshasa, la capitale de la République démocratique du Congo. L’étude sur le sujet a démontré que la maladie est d’origine simiesque, et a donc été transmise par des chimpanzés à l’homme. Si telle est l’origine officielle du sida, beaucoup de personnes n’y croient pas, et pensent que le sida est en fait une maladie qui a été fabriquée en laboratoire. Certains pensent que le peuple africain a servi de cobaye pour tester cette arme biologique et qu’il existe déjà un traitement à cette maladie qui a sciemment été cachée aux pays pauvres. Selon l’IFOP, 32 % des Français croient en des théories du complot sur le sida.",
        proposition: [
            {
                id: 0,
                text: "il a été créé en laboratoire",
            },
            {
                id: 1,
                text: "Par des souris",
            },
            {
                id: 2,
                text: "Par des roches fossiles",
            },
            {
                id: 3,
                text: "Par des Chimpanzés",
            },
        ],
        image: "https://www.allosymptomes.fr/wp-content/uploads/2017/01/sida.jpg",
    },
    {
        question: "Qui a tué le président John F. Kennedy ?",
        type: "unique",
        reponse: 0,
        explication:
            "Le 22 novembre 1963, le président John F. Kennedy était dans un cortège de voitures à Dallas, au Texas, lorsqu’il a été touché par deux balles. Il est décédé quelques instants plus tard à l’âge de 46 ans. Le coupable de ce meurtre était Lee Harvey Oswald et il a été arrêté pour son crime. À peine deux jours après son arrestation, Oswald a été condamné à mort en direct à la télévision. Après les enquêtes menées sur ce meurtre, il a également été découvert qu’Oswald a agi sans complice. Bien évidemment, les adeptes des théories du complot sur le meurtre de Kennedy ne croient pas en cette version. Certains sont persuadés que ce n’est pas Oswald qui a tiré sur le président Kennedy. Certains autres croient qu’il est bel et bien le tireur, mais qu’il a agi selon des ordres donnés par la CIA. Selon cette dernière théorie, les services secrets américains auraient voulu se débarrasser de Kennedy en raison des réactions du président au communisme et à l’invasion de la baie des Cochons.",
        proposition: [
            {
                id: 0,
                text: "Lee Harvey Oswald",
            },
            {
                id: 1,
                text: "La CIA",
            },
            {
                id: 2,
                text: "Kim Il-sung",
            },
            {
                id: 3,
                text: "Les services Secrets Russes",
            },
        ],
        image: "https://upload.wikimedia.org/wikipedia/commons/thumb/5/5c/JFK_limousine.png/1200px-JFK_limousine.png",
    },
    {
        question: "Laquelle ou lesquelles de ces images sont fausses ?",
        type: "images",
        reponse: [0, 1],
        explication:
            "Sur la première photo on peut constater que le sourcil de la dame lui est poussé sur les cheveux et sur la photo deux on peut voir que les immeubles se répètent.",
        proposition: [
            {
                id: 0,
                text: "https://pbs.twimg.com/media/DylymElWwAAKK8f.jpg",
            },
            {
                id: 1,
                text: "https://scontent.fcdg1-1.fna.fbcdn.net/v/t1.6435-9/56222407_2166067607039012_8208994378941726720_n.jpg?_nc_cat=106&ccb=1-3&_nc_sid=730e14&_nc_ohc=dBfYTYco8L8AX-WJ6za&_nc_ht=scontent.fcdg1-1.fna&oh=727c30fc6bffa425cec6cec08cc2d7a0&oe=60E21614",
            },
            {
                id: 2,
                text: "https://boowiki.info/images/a540_11/220px-Camaleonte.jpg",
            },
            {
                id: 3,
                text: "https://www.sciencesetavenir.fr/assets/img/2019/06/11/cover-r4x3w1000-5cffd1a2a5b76-033-ria10-805903-5315.jpg",
            },
        ],
        image: "",
    },
    {
        question: "Cette photo est-elle bien réelle ?",
        type: "unique",
        reponse: 0,
        explication:
            "Dès six heures du matin, en ce jour du mois de juin 1940, Hitler entreprend au pas de course une visite de Paris : l’Opéra, la Concorde, l’arc de triomphe. Sur l’esplanade du Trocadéro, il pose devant les photographes, dos à la Tour Eiffel, puis se dirige vers l’École militaire. Aux Invalides, il se recueille longuement devant le tombeau de Napoléon Ier. Il remonte ensuite vers le jardin du Luxembourg, s’arrête au Panthéon, descend le boulevard Saint-Michel à pied, ses deux gardes du corps à distance, remonte en voiture la place Saint-Michel. Sur l’île de la Cité, il admire la Sainte-Chapelle et Notre-Dame, puis la rive droite (le Châtelet, l’hôtel de ville, la place des Vosges, les Halles, le Louvre, la place Vendôme). Il remonte ensuite vers l’Opéra, Pigalle, le Sacré-Cœur, avant de repartir à 8 h.15. Un survol de la ville complète sa visite. Il ne remettra plus jamais les pieds à Paris. ",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://th.bing.com/th/id/R4a18152f6ea947ef6c3b3ca0273953bf?rik=1HTdUoIQI89igQ&riu=http%3a%2f%2fayay.co.uk%2fbackgrounds%2fhistorical%2fnazi_third_reich%2fhitler-in-front-of-the-eiffel-tower.jpg&ehk=2Ne6IlCwximRlS%2bAnUWa%2fqFA%2bNS2osbFxktJKOOTjiU%3d&risl=&pid=ImgRaw",
    },
    {
        question: "Cette photo est-elle trafiquée ou truquée ?",
        type: "unique",
        reponse: 0,
        explication:
            "En 1934, Robert Kenneth Wilson prit une photo du monstre sur ordre du gouvernement. La photo fit le tour du monde, mais, quelques années plus tard, Robert Kenneth Wilson avoua que la photo était un canular : il s'agissait d'un jouet en plastique. Il a même avoué que c'était lui qui avait tracé sur la berge de prétendues traces du monstre. Selon Peter Costello, qui écrivit en 1977 un livre intitulé A la recherche des monstres lacustres, il s'agirait d'un mammifère qui pourrait être une grosse otarie ou une loutre géante, et que l'on aurait confondu avec un monstre. Après cela, quelques expéditions ont été menées pour tenter d'attraper le monstre. Comme ces expéditions n'ont rien donné, ces chasseurs ont abandonné l'idée du monstre dans le lac… jusqu'à l'apparition d'une vidéo, réalisée en 2007, qui montre un mouvement dans l'eau. Aussitôt l'affaire reprit, et Neil Clark, paléontologue, déclara que le monstre serait seulement un moyen de faire du marketing. En effet, plusieurs rumeurs disent qu'en 1933, le directeur d'un cirque, Bertram Mills, faisait entrer ses éléphants dans l'eau lesquels ne sortaient que le haut de leur tête et de leur dos et la trompe, donnant l'impression d'un animal ayant deux bosses et un long cou.",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "http://townsquare.media/site/482/files/2017/03/Loch-Ness-Monster.jpg",
    },
    {
        question: "Qui était le Fondateur des Illuminés de Bavière, aussi dit Illuminati?",
        type: "unique",
        reponse: 2,
        explication:
            "Adam Weishaupt, né le 6 février 1748 à Ingolstadt en Bavière et mort le 18 novembre 1830 à Gotha, est un théologien et essayiste allemand. Après ses études où il suit l'enseignement des jésuites, il devint en 1775 professeur de droit canonique à l'université d'Ingolstadt. Il est le fondateur de l'ordre des Illuminés de Bavière. Le 1er mai 1776, il fonde le Cercle des Perfectibilistes afin de réunir autour de ces idées nouvelles ses meilleurs étudiants. Sous cette première forme, le groupe de Weishaupt n’est encore qu’une association de penseurs. Il fut initié à la franc-maçonnerie le 2 février 1777 au sein de la loge La Prudence , loge Munichoise qui opérait le système de la Stricte Observance Templière. Rapidement déçu dans ses attentes, il se met à imaginer la création d’un nouvel Ordre.",
        proposition: [
            {
                id: 0,
                text: "Georges Washington",
            },
            {
                id: 1,
                text: "Jean-Jacques Rousseau",
            },
            {
                id: 2,
                text: "Adam Weishaupt",
            },
            {
                id: 3,
                text: "Socrate",
            },
        ],
        image: "https://i.ebayimg.com/images/i/141599547574-0-1/s-l1000.jpg",
    },
    {
        question: "Quelle série télé culte a mis en avant la théorie du complot ?",
        type: "unique",
        reponse: 0,
        explication:
            "Les agents spéciaux du FBI Fox Mulder (David Duchovny) et Dana Scully (Gillian Anderson) sont les enquêteurs travaillant sur des dossiers non classés (les « X-Files »), des affaires non résolues impliquant des phénomènes paranormaux. Mulder croit en l'existence des extraterrestres et au paranormal, tandis que Scully, femme médecin et plus sceptique, est affectée à faire des analyses scientifiques sur les découvertes de Mulder pour l’amener à revenir vers des conclusions habituelles du FBI8. Au cours de ces enquêtes, ils seront confrontés à des monstres, des événements surnaturels, des conspirations et à des phénomènes ayant un lien avec les extraterrestres, présentés comme étant à l'origine de l'enlèvement de la sœur de Mulder, Samantha. Au début de la série, les deux agents deviennent des pions dans un conflit plus vaste, et sont amenés à ne faire confiance qu'à eux-mêmes. Ils développent une relation étroite qui débute par une amitié platonique, puis évolue au fil des saisons, tout cela sous l’œil constamment menaçant de l'inquiétant homme à la cigarette. Cette série oppose en permanence le normal et le paranormal, le possible et l'impossible, le réel et le surnaturel, grâce au scepticisme de Scully et à l'irrationalisme de Mulder.",
        proposition: [
            {
                id: 0,
                text: "X Files",
            },
            {
                id: 1,
                text: "Les envahisseurs",
            },
        ],
        image: "https://www.francetvinfo.fr/image/75izde34i-d5da/1200/450/15762167.jpg",
    },
    {
        question: "Comment est mort Adolf Hitler ?",
        type: "unique",
        reponse: 2,
        explication:
            "Adolf Hitler s'est suicidé le 30 avril 1945 dans son bunker, à Berlin. L'Armée rouge est alors aux portes de la capitale du IIIe Reich. Le führer est en proie à de violentes crises de démence. Son entourage sait la guerre perdue, et la plupart tombent dans la débauche ou tentent de fuir.",
        proposition: [
            {
                id: 0,
                text: "Par une bombes américaine",
            },
            {
                id: 1,
                text: "Guillotiné par Staline",
            },
            {
                id: 2,
                text: "Il s'est suicidé",
            },
            {
                id: 3,
                text: "Par le résistant Français Jean Moulin",
            },
        ],
        image: "https://hanacarmon.weebly.com/uploads/9/8/1/0/98107890/hitler-2_2.jpg",
    },
    {
        question: "Aleix Segura a passé 24 minutes sous l'eau sans respirer",
        type: "unique",
        reponse: 0,
        explication:
            "Le 28 février 2018, lors du 17ème Salon de la Plongée Méditerranéenne à Cornellà, Barcelone, Aleix Segura Vendrell a battu le record du monde Guinness en retenant son souffle sous l’eau pendant 24 minutes et 3 secondes. Cette apnée la plus longue jamais rapportée a été obtenue après l’inhalation d’oxygène pur, différemment des compétitions sportives d’apnée. Aleix Segura est actuellement le champion du monde de l’apnée statique dans tous les circuits d’apnée libre, capable de retenir sa respiration pendant plus de dix minutes dans des conditions normales et a pris le risque de ce défi acrobatique avec le soutien de l’équipe médicale IEM qualifiée supervisée par le Dr. Agustí Ruíz et surveillé entre autres par trois juges de l’AIDA dont Oriol Navarro. Une piscine couverte a été aménagée pour l’événement afin de satisfaire à la fois à la performance de ce champion et la couverture médiatique télévisée.",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://i0.wp.com/www.psmcafe.com/wp-content/uploads/2015/07/Aleix-Segura-Vendrell-portrait.jpg",
    },
    {
        question:
            "L'actrice Marion Cotillard a été persuadée d'être hanté par Edith Piaf après son rôle de La Môme en 2007",
        type: "unique",
        reponse: 0,
        explication:
            "Le fantôme d'Edith Piaf aurait-il hanté Marion Cotillard? C'est ce que semble croire l'actrice. Dans un entretien au Guardian, elle reconnaît avoir tellement donné de sa personne pour son rôle dans La Môme (2007) qu'elle se sentait possédée par l'esprit de la chanteuse. À l'époque, elle s'était rasée la tête et les sourcils, avait appris à tricoter... Bref, elle s'était investie. Si bien que de temps en temps, elle sent encore la présence d'Édith Piaf, notamment quand elle parle. Et la seule solution envisageable, l'exorcisme: «J'ai tout essayé», raconte-t-elle. «J'ai fait des exorcismes avec du sel et du feu. J'ai voyagé jusqu'à Bora Bora pour la fuir. J'ai été au Pérou sur le Machu Picchu afin de procéder à d'anciennes cérémonies chamaniques pour me purifier avant de réaliser pourquoi je n'arrivais pas à la laisser partir. Elle avait été abandonnée dans son enfance. Sa plus grande peur était d'être seule.»",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://i.f1g.fr/media/figarofr/704x396_cropupscale/2014/08/05/PHO9590832c-1c91-11e4-ba2d-ea3c593af34b-805x453.jpg",
    },
    {
        question:
            "Le Navire de Croisière Impérial a été déposé sur une colline à la suite d'un Tsunami en Corée du Sud en 2007, son propriétaire prévoit de creuser un canal pour le ramener sur la mer",
        type: "unique",
        reponse: 1,
        explication:
            "Une rapide recherche sur Google image rétabli la vérité, il s'agit d'un véritable hôtel en forme de bateau, le Sun Cruise Resort And Yatch, construit près de la ville de Jeongdonjin en Corée du sud ",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://static.bayard.io/reporters.1jour1actu.com/prod/games/QUIZ/QUIZ_FAKE_NEWS/assets/QUIZ/images/Q7.jpg",
    },
    {
        question: "Cet article est-il bien réel ?",
        type: "unique",
        reponse: 1,
        explication:
            "Sans faire de recherche sur internet on peut voir que l'auteur de l'article est Fake News Planfor, ensuite le petit pin des landes sur la photo est un montage photo assez mal réalisé, on peut aussi voir sur Google Image que l'image officielle ne comprend pas de Pin sur la table",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://static.bayard.io/reporters.1jour1actu.com/prod/games/QUIZ/QUIZ_FAKE_NEWS/assets/QUIZ/images/Q12.jpg",
    },
    {
        question: "Cette photo est-elle bien réelle ?",
        type: "unique",
        reponse: 0,
        explication:
            "Étonnant mais vrai : c'est à l'ouest de la capitale hollandaise sur l'île de Saint-Martin que se trouve la plage dont tout le monde a entendu parler : Maho Beach. Elle constitue un attrait touristique mondialement connu. Et pour cause, allongé sur une serviette, vous voyez les avions, qui atterrissent à l'aéroport international Princess Juliana, passer à quelques mètres seulement au-dessus de votre tête. Elle est située en bout de piste, ce qui vous permet d'admirer la trajectoire de ces transporteurs comme si vous y étiez. Regarder les avions est devenu une activité si populaire que les horaires de départ et d'arrivée des différentes compagnies sont affichés sur un tableau dans les bars. Le Sunset Beach Bar and Grill dispose même d'une enceinte à l'extérieur qui diffuse les communications radios entre les pilotes et les contrôleurs aériens. Attention tout de même au souffle des avions au décollage, ça décoiffe !",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://www.faxinfo.fr/wp-content/uploads/2015/09/09-09-15-air-france-sint-maarten.jpg",
    },
    {
        question: "Pour vérifier une information faut-il croiser ces sources ?",
        type: "unique",
        reponse: 0,
        explication:
            "Un site d'information n'as pas la science infuse, il peut se tromper donc pour vérifier l'authenticité d'une information, il vaudrait mieux vérifier sur plusieurs sites pour en être certain. ",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://resize.programme-television.ladmedia.fr/rcrop/690,388/img/var/imports/agtv/8/4/7/4187274748_169.jpg",
    },
    {
        question:
            "Qu'est ce qui ne va pas dans ce graphique ?(Plusieurs réponses peuvent être attendue)",
        type: "multiple",
        reponse: [0, 2],
        explication:
            "On peut voir sur ce graphique que le total des pourcentages n'est pas le bon. On peut voir aussi qu'il manque une donnée en pourcentage ce qui donne l'impression que le graphique n'est pas à l'échelle alors qu'il l'est. ",
        proposition: [
            {
                id: 0,
                text: "Les chiffres présentés ne sont pas bon",
            },
            {
                id: 1,
                text: "Le graphique ne représente pas les chiffres à l'échelle",
            },
            {
                id: 2,
                text: "Il manque des chiffres",
            },
            {
                id: 3,
                text: "Le graphique est correct",
            },
        ],
        image: "https://alain-michel.canoprof.fr/eleve/culture-numerique/2.Fake-news-et-manipulations-des-images-02/res/graphique06.jpg",
    },
    {
        question:
            "Qu'est ce qui ne va pas dans ce graphique ?",
        type: "unique",
        reponse: 1,
        explication:
            "Dans cet exemple, on peut voir que les données sont toutes présentes et le total est juste cependant les barres du graphique ne sont pas à l'échelle on peut le constaté avec le pourcentage du partie communiste par rapport au front national",
        proposition: [
            {
                id: 0,
                text: "Les chiffres présentés ne sont pas bon",
            },
            {
                id: 1,
                text: "Le graphique ne représente pas les chiffres à l'échelle",
            },
            {
                id: 2,
                text: "Il manque des chiffres",
            },
            {
                id: 3,
                text: "Le graphique est correct",
            },
        ],
        image: "https://le-bon-sens.com/wp-content/uploads/2017/10/2017-10-12-16.07.59-768x433.jpg",
    },
    {
        question:
            "Dans un débat, faire une diversion en mettant une autre idée en avant s'appelle ? ",
        type: "unique",
        reponse: 0,
        explication:
            "La réponse est agiter le chiffon rouge le but et de parler de quelque chose d'autre pour ne pas répondre à la question posé, pour contrer cette technique il faut insister sur la question pour éviter qu'il détourne le propos en son avantage",
        proposition: [
            {
                id: 0,
                text: "Agiter le chiffon rouge",
            },
            {
                id: 1,
                text: "Sortir le drapeau blanc",
            },
            {
                id: 2,
                text: "La technique de l'homme de paille",
            },
            {
                id: 3,
                text: "La technique de la kipa surgelé",
            },
        ],
        image: "https://i.la-croix.com/729x486/smart/2018/01/30/1200909919/Deux-generations-debat-lentreprise_0.jpg",
    },
    {
        question:
            "Dans un débat peut-on pousser à bout un adversaire afin de décrédibiliser son discours  ?",
        type: "unique",
        reponse: 0,
        explication:
            "C'est une technique fréquemment utilisé par les hommes politique pour déstabiliser l'adversaire, en lui faisant perdre ses moyens il est très difficile de donner un discours crédible de plus on empêche l'adversaire de contre attaquer sous le biais de la colère . ",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://www.leparisien.fr/resizer/Ksk47Y48N6zx59iGln52KOLniic=/932x582/arc-anglerfish-eu-central-1-prod-leparisien.s3.amazonaws.com/public/LHIBR2APNXPPP576PR24OP4UOY.jpg",
    },
    {
        question:
            "Qu'est ce qui ne va pas dans ce graphique ?",
        type: "unique",
        reponse: 3,
        explication:
            "Dans ce graphique nous avons une bonne échelle les chiffres sont correcte est ne manque pas c'est donc un bon graphique.",
        proposition: [
            {
                id: 0,
                text: "Les chiffres présentés ne sont pas bon",
            },
            {
                id: 1,
                text: "Le graphique ne représente pas les chiffres à l'échelle",
            },
            {
                id: 2,
                text: "Il manque des chiffres",
            },
            {
                id: 3,
                text: "Le graphique est correct",
            },
        ],
        image: "https://www.liberation.fr/resizer/nPuvfAzhtIYHRkGPsqr2593NwsE=/400x225/filters:format(png):quality(70)/cloudfront-eu-central-1.images.arcpublishing.com/liberation/NOYFBPFMRXYURQDGYJSI4D4L7A.png",
    },
    {
        question: "Cet article est-il bien réel ?",
        type: "unique",
        reponse: 0,
        explication:
            "Aussi stupide soit-elle, cette pratique existe réellement, la photo ne sert qu'à illustrer l'article, LA source, Le Figaro existe depuis de nombreux années et emploient des journalistes professionnels. De plus l'article est signé et il est même possible de contacter l'auteur, une recherche dans les actualités montre qu'il existe de nombreux articles sur le sujet ",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://static.bayard.io/reporters.1jour1actu.com/prod/games/QUIZ/QUIZ_FAKE_NEWS/assets/QUIZ/images/Q11.jpg",
    },
    {
        question: "Cette photo est-elle bien réelle ?",
        type: "unique",
        reponse: 0,
        explication:
            "En recherchant sur Google Image on apprend que cette photo est authentique, Elle est signée par Michael Paul Smith, un artiste réputé pour ses décorations miniatures. ",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://static.bayard.io/reporters.1jour1actu.com/prod/games/QUIZ/QUIZ_FAKE_NEWS/assets/QUIZ/images/Q8.jpg",
    },
    {
        question: "Cette photo a-t-elle été retouchée ?",
        type: "unique",
        reponse: 0,
        explication:
            "Cette photo est retouchée on peut s'en apercevoir en regardant l'arrière de la photo, ou le parquet est déformé derrière les joueurs. ",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://www.foottheball.com/wp-content/uploads/2017/01/gonzalo-higuain.jpg",
    },
    {
        question: "Quels outils pouvons-nous utiliser pour vérifier une image?",
        type: "multiple",
        reponse: [0, 3],
        explication:
            "TinEye peux nous être utiles pour trouver la provenance d'une images ce qui permet de trouver si c'est du fake la photo originale ou sa provenance et Tungstene est utilisé par le ministère de la défense est permet de détecter des altérations dans une images soit en analysant ses statistiques profonde, soit en recherchant des incohérences dans la chrominances ou la diffusion des rayons lumières. ",
        proposition: [
            {
                id: 0,
                text: "TinEye",
            },
            {
                id: 1,
                text: "Google Analytics",
            },
            {
                id: 2,
                text: "Google Search console",
            },
            {
                id: 3,
                text: "Tungstene",
            },
        ],
        image: "http://4.bp.blogspot.com/-KpIQx_PODAk/VJ7qCwj7bdI/AAAAAAAAPCw/AJwI3oJt4xo/s1600/rouage%2B2.jpg",
    },
    {
        question:
            "Un roman Américain publié en 1898, Futilité ou le naufrage du Titan, raconte avec une grande exactitude la construction et le naufrage du paquebot Titanic... 14 ans AVANT les fait (1912), est-ce réel ?",
        type: "unique",
        reponse: 0,
        explication:
            "Les Similitudes entre le roman et la réalité sont nombreuses : déjà, le nom du navire (Titan/Titanic), la cause de son naufrage (choc avec un iceberg) et sa localisation (Atlantique Nord). Même ses dimension et son poids correspondent, en gros. L'auteur Morgan Robertson, était-il devin ? Non il s'agit simplement de coïncidences, rendues possibles par le fait qu'il possédait une grande connaissance du monde de la marine. Il pouvait donc anticiper des événements probables, comme la construction d'un navire transatlantique gigantesque, et sa rencontre fatale avec un iceberg. Ce n'est pas un seul romancier à avoir ainsi prédit l'avenir avec justesse...",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://static.bayard.io/reporters.1jour1actu.com/prod/games/QUIZ/QUIZ_FAKE_NEWS/assets/QUIZ/images/Q14.jpg",
    },
    {
        question: "Peut-on faire une recherche inversé d'une vidéo Youtube ?",
        type: "unique",
        reponse: 0,
        explication:
            "Grâce à l'outil Youtube DataViewer on peut faire une recherche inversé de chaque image ce qui permet de retrouver l'origine de certaine vidéo",
        proposition: [
            {
                id: 0,
                text: "Vrai",
            },
            {
                id: 1,
                text: "Faux",
            },
        ],
        image: "https://cdn03.nintendo-europe.com/media/images/10_share_images/games_15/nintendo_switch_download_software_1/H2x1_NSwitchDS_YouTube_image1600w.jpg",
    },
];
